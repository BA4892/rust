...
install() {
    ...
    chmod +x "$INSTALL_DIR/libexec/"* 2>/dev/null || true
    chmod +x "$INSTALL_DIR/tool/"* 2>/dev/null || true
    find "$INSTALL_DIR/lib/rustlib" -type d -name 'bin' -exec chmod +x {}/* \; 2>/dev/null || true
    find "$INSTALL_DIR/lib/rustlib" -path '*/gcc-ld/*' -type f -exec chmod +x {} \; 2>/dev/null || true
    find "$INSTALL_DIR/lib" -type f -name '*.so*' -exec chmod +x {} \; 2>/dev/null || true
